### Acknowledge source codes from the TWAS / FUSION site by Gusev lab: http://gusevlab.org/projects/fusion/
...
option_list = list(
  ...
  make_option("--force_model", action="store", default=NA, type='character',
              help="Force specific predictive model to be used, no flag (default) means select most significant cross-val. Options: top1,enet."),
...
N = nrow(wgtlist)
out.tbl = data.frame( "PANEL" = rep(NA,N) , "FILE" = character(N) , "ID" = character(N) , "CHR" = numeric(N) , "P0" = character(N) , "P1" = character(N) ,"HSQ" = numeric(N) , "BEST.GWAS.ID" = character(N) , "BEST.GWAS.Z" = numeric(N) , "PQTL.ID" = character(N) , "PQTL.R2" = numeric(N) , "PQTL.Z" = numeric(N) , "PQTL.GWAS.Z" = numeric(N) , "NSNP" = numeric(N) , "NWGT" = numeric(N) , "MODEL" = character(N) , "MODELCV.R2" = character(N) , "MODELCV.PV" = character(N) , "PWAS.Z" = numeric(N) , "PWAS.P" = numeric(N) , stringsAsFactors=FALSE )
...
for ( w in 1:nrow(wgtlist) ) {
	...
	pqtlmod = colnames(wgt.matrix) == "top1"
	toppqtl = which.max( wgt.matrix[,pqtlmod]^2 )
	
	if ( cur.FAIL || sum(pqtlmod) == 0 || length(toppqtl) == 0 || is.na(toppqtl) ) {
		out.tbl$PQTL.ID[w] = NA
		out.tbl$PQTL.R2[w] = NA
		out.tbl$PQTL.Z[w] =  NA
		out.tbl$PQTL.GWAS.Z[w] = NA
	} else {
		# out.tbl$PQTL.ID[w] = names( toppqtl )
		out.tbl$PQTL.ID[w] = rownames(wgt.matrix)[toppqtl]
		out.tbl$PQTL.R2[w] = cv.performance[1,pqtlmod]
		out.tbl$PQTL.Z[w] = wgt.matrix[ toppqtl , pqtlmod ]
		out.tbl$PQTL.GWAS.Z[w] = cur.Z[ toppqtl ]
		...
		if( opt$caviar ) {
			...
			write.table( format(wgt.matrix[,pqtlmod],digits=3) , quote=F , col.names=F , sep='\t' , file = paste( cav.out , ".PQTL.Z" , sep='') )
			write.table( format(cur.Z,digits=3) , quote=F , col.names=F , sep='\t' , file = paste( cav.out , ".GWAS.Z" , sep='') )
		}
		
		# perform JLIM test
		if ( opt$jlim ) {
			jlim.lambda.t = jlimR:::calc.stat( cur.Z , wgt.matrix[,pqtlmod] , cur.LD , cur.LD , jlim.r2res )
			# TODO : Sample permuted pQTL Z-scores from the MvN using LD matrix
			jlim.nulldist = jlimR:::perm.test( cur.Z , wgt.matrix[,pqtlmod], permmat, cur.LD, cur.LD, jlim.thresholdingP, jlim.r2res, jlim.lambda.t)
	...
	if ( !is.na(opt$coloc_P) && !is.na(out.tbl$PWAS.Z[w]) && out.tbl$PWAS.P[w] < opt$coloc_P && !is.na(wgtlist$N[w]) ) {
		b1 = wgt.matrix[,pqtlmod] / sqrt(wgtlist$N[w])
...
# compute p-value
#out.tbl$PWAS.P = 2*(pnorm( abs(out.tbl$PWAS.Z) , lower.tail=F))
